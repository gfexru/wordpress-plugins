...
This email was sent from the {site_title} blog.";
		...
		$newpath = strstr($dstpath, 'uploads');
...
This email was sent from the {site_title} blog.";
		...
		if($upload_email_user == 1 && current_user_can( 'manage_options' ) ){
			//check and see if this is one of the default user folder


			$query = "SELECT * FROM `" . FM_TABLE_NAME . "` WHERE `folder` = '{$complete_path}' AND `meta` = 'red_fm_default' ";
