...
class PrimeMoverExportUtilities
{
    ...
    public function getPluginsToExport($export_data = [])
    {
        ...
        $plugins = apply_filters('prime_mover_exclude_plugins_in_export', $plugins);
        if ( ! is_array($plugins) ) {
            return [];
        }
    ...
    public function updateUserRoleToRandomPrefix($data = '')
    {
        ...
        $export_blog_id = $this->getExporter()->getSystemInitialization()->getExportBlogID();
        $current_prefix = $this->getExporter()->getSystemFunctions()->getDbPrefixOfSite($export_blog_id);
        $updated_prefix = $this->getExporter()->getSystemInitialization()->getRandomPrefix();
    ...
    public function exportDialog($blog_id = 0)
    {
        ...
        <div style="display:none;" id="js-prime-mover-export-dialog-confirm-<?php echo esc_attr($blog_id); ?>" 
        	title="<?php esc_attr_e('Export Options', 'prime-mover')?>" >            
            	...
            	<?php }?>          		  	
    		  	<p><label><input autocomplete="off" type="radio" name="prime-mover-export-mode-<?php echo esc_attr($blog_id); ?>" value="dbonly_export"> 
				...
				<p><label><input autocomplete="off" type="radio" name="prime-mover-export-mode-<?php echo esc_attr($blog_id); ?>" value="db_and_media_export"> 
				...
				<p><label><input autocomplete="off" type="radio" name="prime-mover-export-mode-<?php echo esc_attr($blog_id); ?>" value="development_package"> 
				...
				<p><label><input autocomplete="off" checked="checked" type="radio" name="prime-mover-export-mode-<?php echo esc_attr($blog_id); ?>" value="complete_export_mode"> 
				...
				<h3><?php esc_html_e('Export type (required)', 'prime-mover' )?></h3>
    		        ...
    		        <div style="display:none" class="prime-mover-export-to-multisite-div" id="js-prime-mover-export-to-multisite-div-<?php echo esc_attr($blog_id); ?>">
    				    ...
    				    <p class="prime-mover-enter-multisite-id"><?php echo esc_html__('Enter multisite target blog ID (integers only)', 'prime-mover'); ?>.</p>
    				    <p class="prime-mover-get-blogid-link"><a class="prime-mover-external-link" target="_blank" href="<?php echo esc_url(PRIME_MOVER_GET_BLOGID_TUTORIAL); ?>">
