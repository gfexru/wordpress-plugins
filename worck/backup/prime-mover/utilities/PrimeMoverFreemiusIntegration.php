...
{    
    ...
    {      
        ...
        add_action('prime_mover_after_db_processing', [$this, 'activateOnlyOneVersion'], 100);
     ...
     * import based on customer information
     ...
     */
    public function activateOnlyOneVersion()
    {
        if ( ! $this->getSystemAuthorization()->isUserAuthorized()) {
            return;
        }
        ...
        if ( ! $this->getSystemFunctions()->getPluginFullPath(PRIME_MOVER_DEFAULT_FREE_BASENAME, true) ) {
            $free_exist = false;
            deactivate_plugins(PRIME_MOVER_DEFAULT_FREE_BASENAME, true);
        }
        if ( ! $this->getSystemFunctions()->getPluginFullPath(PRIME_MOVER_DEFAULT_PRO_BASENAME, true) ) {
            $pro_exist = false;
            deactivate_plugins(PRIME_MOVER_DEFAULT_PRO_BASENAME, true);
        }
        
        if ($free_exist && is_plugin_active(PRIME_MOVER_DEFAULT_FREE_BASENAME)) {
            $free_active = true;
        }
        
        if ($pro_exist && is_plugin_active(PRIME_MOVER_DEFAULT_PRO_BASENAME)) {
            $pro_active = true;
        }
        ...
        if ( ! $free_active && ! $pro_active ) {            
            do_action('prime_mover_log_processed_events', 'ERROR: Prime Mover encounters fatal error and deactivated.', 0, 'import', 'activateOnlyOneVersion', $this);
            do_action( 'prime_mover_shutdown_actions', ['type' => 1, 'message' => esc_html__('Prime Mover encounters fatal error and deactivated.')] );
            return wp_die();
        }        
        if ( ! $do_deactivation ) {
            do_action('prime_mover_log_processed_events', 'Only one Prime Mover version is active, this is correct.. skipping deactivation.', 0, 'import', 'activateOnlyOneVersion', $this);
            return;
        }
        if ($this->isCustomer()) {
            do_action('prime_mover_log_processed_events', 'Logged-in customer deactivating free version.', 0, 'import', 'activateOnlyOneVersion', $this);
            deactivate_plugins(PRIME_MOVER_DEFAULT_FREE_BASENAME, true);
        } else {
            do_action('prime_mover_log_processed_events', 'Non-customer logged-in deactivating pro version.', 0, 'import', 'activateOnlyOneVersion', $this);
            deactivate_plugins(PRIME_MOVER_DEFAULT_PRO_BASENAME, true);
        }
    }
    ...
    /**
     * Checks if multisite blog_id is licensed using API
     * @param boolean $ret
     ...
     * @return array|mixed[]
     */
    private function getAllFreemiusSDKOptions($blog_id = 0)
    {
        $this->getSystemFunctions()->switchToBlog($blog_id);
     ...
     * @param number $blog_id
     */
    private function deleteAllFreemiusOptions($blog_id = 0)
    {
        if ( ! $this->getSystemAuthorization()->isUserAuthorized()) {
            return;
        }
