...
class PrimeMoverImportUtilities
{
    ...
    public function showCoreImportButton($blog_id = 0) 
    {
        ...
        $mobile_class = '';
        $button_text = sprintf( esc_html__('Restore blog ID: %d', 'prime-mover'), $blog_id );
        if (wp_is_mobile()) {
            $mobile_class = 'prime_mover_is_mobile';
            $button_text = esc_html__('Restore', 'prime-mover');
        }
        if ( ! is_multisite() ) {
            $button_text = esc_html__('Import', 'prime-mover');
        }
    ?>
       <label for="js-prime_mover_importing_blog_<?php echo esc_attr($blog_id) ; ?>" id="js-prime-mover-browseupload-label-<?php echo esc_attr($blog_id) ; ?>" class="button-primary prime-mover-fileupload-label">
    ...
    public function importWarning($blog_id = 0)
    {
        ...
        $server_upload_limits = "<code>upload_max_filesize = $upload_max_filesize</code> | <code>post_max_size = $post_max_size</code>";
        ?>
        <div style="display:none;" id="js-prime-mover-import-warning-confirm-<?php echo esc_attr($blog_id); ?>" title="<?php esc_attr_e('Heads Up!', 'prime-mover')?>"> 
          ...
          <?php echo esc_html__('It will take around ', 'prime-mover') . ' ' . "<span id='js-prime-mover-computed-uploadtime'></span>" . ' ' . 
              sprintf(esc_html__('to upload this package based on your upload speed & server upload parameters (%s) .', 'prime-mover'), $server_upload_limits); ?></p>
          ...
          <p class="js-prime-mover-upload-option-selected prime-mover-upload-option-selected">
              ...
              <?php echo esc_html__('Upgrade to PRO version to use faster migration options.', 'prime-mover');?></a></p>  
          <?php 
          }
          ?>        
    	  <p><?php esc_html_e('Import process deletes old site and will be replaced with the imported package.', 'prime-mover') ?></p>
    	  <p><?php esc_html_e('Please verify that the following restoration information is correct:', 'prime-mover') ?></p>
    	  ...
    	  <p><span class="js-prime-mover-encrypted-package-note prime-mover-encrypted-package-note"></span> <?php esc_html_e('To be safe, make sure you have backups ready.', 'prime-mover'); ?> <strong><?php  esc_html_e('Are you sure you want to do this?', 'prime-mover'); ?></strong></p>   	  	
        </div>
    <?php
    }
    ...
    private function handleThemeImport($template_path_wpcontent = '', $template_path_package = '', $blog_id = 0, $start = 0, $processed = 0, $import_data  = [], $copying_what = 'parent')
    {
        ...
        if (!$processed) {
            $theme_make_directory_result = $wp_filesystem->mkdir($template_path_wpcontent);
