...
{   
    ...
    protected function isOpenSSLCustomer($blog_id = 0)
    {
        ...
        if (is_multisite() && $customer) {
            /**
             * We are on a multisite and this customer is licensed
             */
            if (true === apply_filters('prime_mover_multisite_blog_is_licensed', false, $blog_id)) {
                /**
                 * However we need to check the license on a sub-site level
                 * This appears TRUE
                 */
                $customer = true;
            } else {
                /**
                 * Subsite is not licensed, return false
                 */
                $customer = false;
            }
        }
     ...
     */
    public function maybeEncryptSetting($value, $encrypt = false)
    {
        if (!$encrypt ) {
            return $value;
        }
     ...
     */
    public function maybeDecryptSetting($value, $decrypt = false)
    {
        if (!$decrypt ) {
            return $value;
        }
