...
{         
     ...
     */
    public function getExcludedUserMetaKeys()
    {
        return apply_filters('prime_mover_excluded_meta_keys', ['session_tokens']);
    }
     ...
     * @return boolean
     */
    public function isUserMetaExcluded($meta_key = '')
    {
       return in_array($meta_key, $this->getExcludedUserMetaKeys());    
    }
    ...
    protected function doUserMetaImport($user_array = [], $user_id_updated = 0, $users_imported = 0, $ret = [], $blog_id = 0, $start_time = 0, $orig_pos = 0, $do_meta_loops = true)
    {
        ...
        if ($do_meta_loops) {
            foreach($user_array as $meta_key => $meta_value) {
                if ($this->isUserMetaExcluded($meta_key)) {
                    continue;
                }
