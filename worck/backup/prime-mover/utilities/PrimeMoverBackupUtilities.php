...
{       
     ...
     * @return boolean
     */
    public function maybeRefreshBackupData($backups_hash_db = '', $current_backup_hash = '', $markup = false)
    {                
        if ($this->isUserRequestingBackupRefresh($markup)) {
            return true;
        }
     ...
     */
    protected function isUserRequestingBackupRefresh($markup = false)
    {
        ...
        if ( $this->getSystemAuthorization()->isUserAuthorized() && isset($get['prime_mover_refresh_backups']) && 
            check_admin_referer('refresh_backups_' . $current_user_id, 'prime_mover_refresh_backups') && 
            $this->getSystemFunctions()->isRefererBackupMenu()) {
                $this->setIsRefreshingBackup(true);
                return true;
        }
        return apply_filters('prime_mover_force_backup_refresh', false, $markup);
    }
