...
{        
    ...
    public function disableExportButtonMenuMainSite($addnewbackup_button = [], $blog_id = 0, $enabled = false, $original_button_markup = [])
    {
        ...
        if ($this->getSystemFunctions()->isMultisiteMainSite($blog_id) && $enabled) {
            ...
            $note = esc_html__('Exporting main site is a PRO feature. Please upgrade to use this feature.', 'prime-mover');
    ...
    public function noMainSiteSupportByDefault($new_parameters = [], $original_parameters = [], $blog_id = 0, $link_active = false)
    {
        ...
        if ($this->getSystemFunctions()->isMultisiteMainSite($blog_id) && $link_active) {
            ...
            $note = esc_html__('Restoring main site is a PRO feature. Please upgrade to use this feature.', 'prime-mover');
            $class = 'button';
     ...
     * @moved
     */
    public function generateDownloadURLForClipBoard($sanitized_name = '', $blog_id = 0, $ret_array = false)
    {
        $ret = '';
        ...
        return $this->getSystemInitialization()->getDownloadURLGivenParameters($download_args);
    }
    ...
    public function validateBackups($backups = [], $blog_id = 0, $current_backup_hash = '')
    {
        ...
        if ( $this->getBackupUtilities()->maybeRefreshBackupData($backups_hash_db, $current_backup_hash) ) {
            $refresh = true;
        }
