        ...
        $blog_id = 0, $processed = 0, $delete_source = true, $source_id = 'default', $resource = [], $ret = []) 
    {
        ...
        foreach ( (array) $dirlist as $filename => $fileinfo ) {
            ...
            if ( in_array( $filename, $skip_list, true ) ) {
                do_action('prime_mover_log_processed_events', "$filename is in skip list, skipping copy.", $blog_id, 'import', __FUNCTION__, $this, true);
            ...
            if ( 'f' == $fileinfo['type'] ) {                
                ...
                if (in_array($ext, $excluded_filetypes, true)) {
                    do_action('prime_mover_log_processed_events', "$ext is skip extensions, skipping copy.", $blog_id, 'import', __FUNCTION__, $this, true);
                    $ret['copydir_processed']++;
                    continue;
                }             
                ...
                if (is_array($processfile) && !isset($processfile['copychunked_offset'])) {
                    ...
                    do_action('prime_mover_log_processed_events', 'ERROR copying file ' . $from . $filename . ", retrying with improved permissions.", $blog_id, 'import', __FUNCTION__, $this, true);
                    ...
                    if ( ! $this->processFile(
                        ...
                        do_action('prime_mover_log_processed_events', 'STILL HAVING ERROR copying file ' . $from . $filename . " , bailing out.", $blog_id, 'import', __FUNCTION__, $this, true);
                ...
                if ( ! $wp_filesystem->is_dir( $to . $filename ) ) {
                    if ( ! $wp_filesystem->mkdir( $to . $filename, FS_CHMOD_DIR ) ) {                        
                        do_action('prime_mover_log_processed_events', 'ERROR: Unable to create directory: ' . $to . $filename . " as requisite for copying, bail out.", $blog_id, 'import', __FUNCTION__, $this, true);
                ...
                if (is_wp_error($result))  {
                    do_action('prime_mover_log_processed_events', 'ERROR: copyDir FROM ' . $from . $filename . ' TO ' . $to . $filename, $blog_id, 'import', __FUNCTION__, $this, true);
                    return $result;                              
                }
