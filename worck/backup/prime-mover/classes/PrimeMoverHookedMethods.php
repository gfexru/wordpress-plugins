...
class PrimeMoverHookedMethods
{
     ...
     */
    private function getBlogAddress($blog_id = 0)
    {
        if (is_multisite()) {
     ...
     */
    private function isColumnsMatched($mode = 'export', $column_name = '', $blog_id = 0)
    {
     ...
     */
    private function maybeLoadMigrationSection($column_name = '', $blog_id = 0, $mode = 'export')
    {
        if ( ! is_multisite() ) {
            return true;
        }
    ...
    public function primeMoverExportSection($column_name = '', $blog_id = 0)
    {
        ...
        if (true === $this->getSystemChecks()->primeMoverEssentialRequisites()) {
			...
			<input	name="prime_mover_exportbutton" 
					...
					class="button-primary prime_mover_exportbutton js-prime_mover_exportbutton"
            ...
            ?>	
		    <a title="<?php echo esc_attr__('Upgrade to PRO to create backup or export the main site.', 'prime-mover'); ?>" href="<?php echo esc_url($upgrade_url);?>" class="button"><?php esc_html_e( 'Upgrade to PRO', 'prime-mover' ); ?></a>			
			<?php                          
            }
        }
    }
     ...
     */
    private function generateExportButtonText($blog_id = 0)
    {
        $text = sprintf(('%s : %d'), esc_html__('Export blog ID', 'prime-mover'), esc_attr($blog_id));
        if (wp_is_mobile() || ! is_multisite() ) {
            $text = esc_html__('Export', 'prime-mover');
        }
	...
	<?php
            ...
            if (!$rendered && $this->getSystemFunctions()->isMultisiteMainSite($blog_id) && $this->isColumnsMatched('import', $column_name, $blog_id)) {
                $upgrade_url = network_admin_url( 'admin.php?page=migration-panel-settings-pricing');
                ?>
		        <a title="<?php echo esc_attr__('Upgrade to PRO to migrate or restore the main site.', 'prime-mover'); ?>" href="<?php echo esc_url($upgrade_url);?>" class="button"><?php esc_html_e( 'Upgrade to PRO', 'prime-mover' ); ?></a>			
			<?php                          
            }
        }
    }
     ...
     */
    private function maybeWeAreNotOnSitesPage($hook = '')
    {
        ...
        wp_localize_script(
            ...
            'prime_mover_js_ajax_renderer',
                apply_filters( 'prime_mover_ajax_rendered_js_object', [
                        ...
                        'prime_mover_export_nonce' => wp_create_nonce('prime_mover_export_nonce'),
                        'prime_mover_import_nonce' => wp_create_nonce('prime_mover_import_nonce'),
                        'prime_mover_uploads_nonce' => wp_create_nonce('prime_mover_uploads_nonce'),
                        'prime_mover_import_progress_nonce' => wp_create_nonce('prime_mover_import_progress_nonce'),
                        'prime_mover_export_progress_nonce' => wp_create_nonce('prime_mover_export_progress_nonce'),
                        'prime_mover_decryption_check_nonce' => wp_create_nonce('prime_mover_decryption_check_nonce'),
                        'prime_mover_errorlog_nonce' => wp_create_nonce('prime_mover_errorlog_nonce'),
                        'prime_mover_deletetmpfile_nonce' => wp_create_nonce('prime_mover_deletetmpfile_nonce'),
     ...
     * @tested PrimeMoverFramework\Tests\TestPrimeMoverHookedMethods::itReturnsErrorWhenFileInfoDisabled
     */
    public function multisiteShowNetworkAdminNotice()
    {
		...
		<?php
            ...
            if (false === $this->getSystemChecks()->primeMoverCheckIfFileinfoExtensionEnabled()) {
                //No fileinfo extension ?>
			<p><?php echo esc_html__('Prime Mover plugin is not yet ready to use. Error: Requires fileinfo PHP extension. Please enable it.', 'prime-mover'); ?></p>
		<?php
            }
        ?>
		</div>
   ...
   <?php      
    ...
    public function addMenuPage()
    {
        ...
        add_menu_page( esc_html__('Prime Mover Control Panel', 'prime-mover'), esc_html__('Prime Mover', 'prime-mover'),
    ...
    public function addMenuPageCallBack()
    {
        ?>
      <div class="wrap">
         <h1><?php esc_html_e('Prime Mover Control Panel', 'prime-mover')?></h1>
