...
class PrimeMoverImporter implements PrimeMoverImport
{
     ...
     */
    private function cleanUpMediaIndexes($ret = [])
    {
     ...
     */
    private function killBlockingProcesses()
    {       
        global $wpdb;
     ...
     */
    private function convertOldTableNamesToNew($ret = [], $clean_tables = [])
    {
        if (! $this->getSystemAuthorization()->isUserAuthorized()) {
            return;
        }
    ...
    {        
        ...
        foreach ($all_tables as $k => $table) {           
            $table_rows_count[] = $wpdb->get_var("SELECT count(*) FROM {$table}");
            unset($all_tables[$k]);           
     ...
     */
    private function cleanUpTableRowsMeta($ret = [])
    {
    ...
    {        
        ...
        /**
         * EMERSON EXPERIMENT
         * $replaceables will be inputted to a custom filter for backslashing
         * $ret['main_search_replace_replaceables'] SHOULD NEVER CONTAIN BACKSLASH FOR DATABASE PORTABILITY
         */
        $replaceables = $ret['main_search_replace_replaceables'];  
        
        /**
         * EMERSON EXPERIMENT
         * THERE IS AN ISSUE ON JSON BACKSLASH REPLACEABLE COMMONLY USED BY PAGE BUILDERS SUCH AS ELEMENTOR
         * TO MINIMIZE COMPLEXITY, LET'S INJECT THIS CHANGE IN THE FINAL FILTER...
         * 
         */
        ...
        if ( ! empty($ret['srch_rplc_completed'] ) ) {
            ...
            /**
             * EMERSON EXPERIMENT
             * SEARCH REPLACE ONGOING, NEED TO RETRY AGAIN!
             */
