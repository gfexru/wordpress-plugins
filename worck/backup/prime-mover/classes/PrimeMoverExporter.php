...
class PrimeMoverExporter implements PrimeMoverExport
{
    ...
    private function handleRandomdBPrefix($ret = [], $blogid_to_export = 0)
    {
        ...
        if (is_multisite() && !$this->getSystemFunctions()->isMultisiteMainSite($blogid_to_export)) {               
                $updated_prefix = $updated_prefix . $blogid_to_export . '_'; 
        }
    ...
    {        
        ...
        $use_mysqldump_php = $this->maybeUseMySQLDumpPhp($ret);  
     ...
     * @param array $ret
     * @param number $db_dump_start
     * @return string[]|number|string[]|NULL[]|boolean[]|string[]|NULL[]
     */
        ...
        try {
            ...
            foreach ($clean_tables as $key => $table) {
                ...
                while ($dumped_rows = $this->triggerMySQLDumpPHP($mysql_server, $table, $filter_export_data, $target_path, $i, $batch_size, $clean_tables, $table_count)) {
                    ...
                    $i = $i + $batch_size;                    
                    $dumped_log[] = $dumped_rows;
                    $retry_timeout = apply_filters('prime_mover_retry_timeout_seconds', PRIME_MOVER_RETRY_TIMEOUT_SECONDS, 'executeDumpUsingPHP');
                    if (microtime(true) - $db_dump_start > $retry_timeout) {
     ...
     */
    private function countAllTableRowsDb($tables = [])
    {
        global $wpdb;
        $rows = 0;
        $tables_count = [];
     ...
     */
    private function getTablesToExport($blogid_to_export = 0)
    {
        ...
        if ($this->getSystemFunctions()->isMultisiteMainSite($blogid_to_export)) {              
     ...
     * @return mixed
     */
    private function triggerMySQLDumpPHP($mysql_server = '', $table = '', $filter_export_data = false, $target_path = '', $i = 0, $batch_size = 10, $clean_tables = [], $table_count = 0)
    {
        ...
        if ($filter_export_data) {
            ...
            return $dump->start($encryted_streamer, true, $i, $batch_size, $table, $clean_tables, $table_count);
        } else {
            return $dump->start($target_path, true, $i, $batch_size, $table, $clean_tables, $table_count);
        } 
    }
     ...
     */
    private function getMySQLDumpPHPParameters($clean_tables = [])
    {
        /**
    ...
    {        
        ...
        $skip = [$exportdir_slug, $importdir_slug, $download_tmp_slug];
    ...
    {        
        ...
        $download_nonce = wp_create_nonce('prime_mover_download_package_' . $blogid_to_export);
        ...
        $download_url = $this->getSystemInitialization()->getDownloadURLGivenParameters($args);
