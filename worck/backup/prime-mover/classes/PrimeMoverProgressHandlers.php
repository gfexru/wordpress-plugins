...
{       
    ...
    public function maintenanceModeControl()
    {
        ...
        if ( ! wp_verify_nonce( $nonce, 'prime-mover-disable-maintenance' ) ) {
            return;
        } 
        if ('off' === $maintenance) {
    ...
    public function primeMoverReactivateAllPlugins($delete_tmpfile_post = [])
    {
        ...
        activate_plugins($singlesite_plugins, '', false, true);
    ...
    public function primeMoverDeactivateAllPlugins($blogid_to_import = 0, $import_initiated = false)
    {
        ...
        deactivate_plugins($thirdparty_plugins, true, false);        
    }
    ...
    {
        $args['prime_mover_import_shutdown_nonce'] = wp_create_nonce('prime_mover_import_shutdown_nonce');
        $args['prime_mover_export_shutdown_nonce'] = wp_create_nonce('prime_mover_export_shutdown_nonce');
    ...
    public function commonShutDownProcessor($mode = 'import')
    {
        ...
        if ( ! wp_verify_nonce($shutdown_post[$shutdown_nonce], $shutdown_nonce)) {
            wp_send_json( $response );
        }
    ...
    public function publicMaintenanceDuringRestore()
    {
        ...
        $nonce = wp_create_nonce( 'prime-mover-disable-maintenance' );
    ...
    public function commonProgressProcessor($mode = 'import')
    {
        ...
        if ( ! wp_verify_nonce($progress_post[$progress_nonce], $progress_nonce)) {
            return wp_send_json( $response );
        }
