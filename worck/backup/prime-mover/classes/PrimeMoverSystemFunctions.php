...
{    
    ...
    public function getExportPathOfThisSubsite($blog_id = 0)
    {
        ...
        $blog_id	= (int) $blog_id;
        if ($blog_id > 0) {
            $ret= $this->getSystemInitialization()->getMultisiteExportFolderPath() . $blog_id . DIRECTORY_SEPARATOR;
        }
        return $ret;
    }
    /**
    ...
    public function generateImportReplaceables($ret = [])
    {
        ...
        $add_slashed_origin_wp_root = str_replace('/', '\/', $origin_wp_root);
        $add_slashed_target_wp_root = str_replace('/', '\/', $target_wp_root);
        $add_slashed_origin_wp_root_trim = str_replace('/', '\/', $rtrimmed_origin_wp_root);
        $add_slashed_target_wp_root_trim = str_replace('/', '\/', $rtrimmed_target_wp_root);
    ...
    public function cleanDbTablesForExporting($inputs = [], $blog_id = 0, \wpdb $wpdb, $mode = 'export')
    {
        ...
        if (is_multisite() && !$this->isMultisiteMainSite($blog_id)) {
            $multisite = true;            
            $separator = '_';
        } 
        ...
        if ('export' === $mode ) {
            ...
            if ($multisite) {
                $match_prefix = $db_prefix . $blog_id;
            }
        ...
        foreach ($inputs as $k => $table_detail) {
    ...
    public function activatePrimeMoverPluginOnly($blog_id = 0)
    {
        ...
        update_option($active_plugins_option, $active_plugins);
        $this->restoreCurrentBlog();        
     ...
     * Uses finfo_open instead of relying on $_FILES['type']
     ...
     */
    public function isMultisiteMainSite($blog_id = 0)
    {
     ...
     * @return boolean
     */
    public function activatePlugin($blog_id = 0, $plugin_path_target = '', $plugin = '', $network_activated = false)
    {
        $this->switchToBlog($blog_id);
