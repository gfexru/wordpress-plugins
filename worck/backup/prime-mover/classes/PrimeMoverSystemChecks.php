...
{       
    ...
    public function addSchemeToSystemFootprint(array $footprint, $blogid_to_export = 0)
    {
        ...
        if (is_ssl()) {
            $footprint['scheme'] = PRIME_MOVER_SECURE_PROTOCOL;
        } else {
            $footprint['scheme'] = PRIME_MOVER_NON_SECURE_PROTOCOL;
        }
        return $footprint;
    }
     ...
     * @compatible 5.6
     */
    public function primeMoverCheckIfFileinfoExtensionEnabled()
    {
    ...
    public function primeMoverGenerateSystemFootprint($blogid_to_export = 0, $tmp_folderpath = '', $ret = [])
    {
        ...
        if ((isset($url_parsed['host'])) && (!empty($url_parsed['host']))) {
            $site_url_parsed .= $url_parsed['host'];
        }
        if ((isset($url_parsed['path'])) && (!empty($url_parsed['path']))) {
            $site_url_parsed .= $url_parsed['path'];
        }
        if (!empty($site_url)) {
        ...
        if (defined('ABSPATH')) {
            $footprint['wp_root'] = get_home_path();
        }
    ...
    public function addPluginActionLinks($actions = [], $plugin_file = '', $plugin_data = [], $context ='')
    {
        ...
        if ($this->getSystemInitialization()->getPluginUri() === $pluginuri && $this->getSystemInitialization()->getPluginFile() === $pluginfile) {
            $actions[] = '<a class="edit" href="' . esc_url_raw(network_admin_url('sites.php')) . '">' . esc_html__('Migrate/Backup Sites', 'prime-mover') . '</a>';
        }
        return $actions;
    }
