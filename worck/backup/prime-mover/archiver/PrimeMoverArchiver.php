...
{         
        ...
        $archiving_params = [], $mode = 'wb', $encryption_key = '')
    {
        ...
        if (!file_exists($filelist) || ! $tar_path || !file_exists($rootpath)) {
            $ret['error'] = esc_html__("Incorrect WPRIME add directory configuration.", 'prime-mover');
            return $ret;
        }        
        ...
        while(!feof($handle)) {
            ...
            do_action('prime_mover_log_processed_events', "Processing $line for archiving", $blog_id, 'export', __FUNCTION__, $this);
            if (!$line) {
                continue;
            }
            if (!file_exists($line)) {
    ...
    protected function maybeExcludeMedia($exporting_mode = '', $line = '', $exclusion_rules = [], $rootpath = '', $blog_id = 0)
    {
        ...
        if (apply_filters('prime_mover_media_resource_is_excluded', false, $line, $exclusion_rules, $rootpath, $exporting_media)) {
