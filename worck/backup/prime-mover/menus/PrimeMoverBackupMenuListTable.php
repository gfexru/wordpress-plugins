...
{    
    ...
    protected function getMigrateSiteBackup($item = [])
    {
        if ( false === apply_filters('prime_mover_is_loggedin_customer', false)) {
            ...
            $note = esc_attr__('Upgrade to PRO to save time migrating sites.', 'prime-mover');
            $url = "#";
            $class = "button disabled prime-mover-menu-button";
            $link_text = esc_html__('PRO version only', 'prime-mover');
            $clipboard_span = '';
            $data_clipboard_id = '';
            ...
            $note = sprintf(esc_attr__('Copy restore URL to clipboard to migrate this to another %s', 'prime-mover'), $item['package_type']);
            
            $class = "button prime-mover-menu-button js-prime-mover-clipboard-button-responsive js-prime-mover-copy-clipboard-menu prime-mover-copy-clipboard-menu-button";
            $link_text = esc_html__('Copy restore URL', 'prime-mover');
            $data_clipboard_id = esc_attr($item['sanitized_package_name']);
            $clipboard_span = '<span id="' . $data_clipboard_id . '" class="prime-mover-clipboard-key-confirmation-menu js-prime-mover-copy-clipboard-menu">' . esc_html__('Copied', 'prime-mover') . ' !' . '</span>';           
        }
    ...
    public function get_columns()
    {
        $columns = [
            ...
            'migrate' => esc_html__('Migrate site', 'prime-mover'),
    ...
    public function get_sortable_columns() 
    {
        ...
        if ( ! is_multisite() ) {
            unset($sortable_columns['blog_id'] );
        }
    ...
    {        
        ...
        return ('delete' === $this->current_action() && $this->getSystemAuthorization()->isUserAuthorized() && isset($input['_wpnonce']) && wp_verify_nonce( $input['_wpnonce'], 'bulk-prime_mover_backups'));
    }
    ...
    {        
        ...
        if ( $this->getBackupUtilities()->maybeRefreshBackupData($backups_hash_db, $current_backup_hash, true) ) {
            $refresh = true;
        }
        ...
        foreach ($backups as $filename => $backup_meta) {  
            ...
            $backup_array['download_url'] = $this->getComponentUtilities()->generateDownloadURLForClipBoard($sanitized_name, $blog_id);
    ...
    private function restoreFreeBackupParameters($blog_id = 0, $backup_filepath = '', $encryption_status = 'false')
    {
        ...
        if ('true' === $encryption_status) { 
            $note = esc_html__('Restoring encrypted package is a PRO feature. Please upgrade to restore this package.', 'prime-mover');
            $class = "button";
            $url = $this->getSystemInitialization()->getUpgradeUrl();
            $link_text = esc_html__('Upgrade to PRO', 'prime-mover');
            $link_active = false;
        }
