<?php
namespace LocoAutoTranslateAddonPro\LocoAutomaticTranslateAddonProUpdaterBase;

if(!class_exists("LocoAutomaticTranslateAddonProUpdaterBase")) {
	class LocoAutomaticTranslateAddonProUpdaterBase {
    	public $key = "354C1DA5404551D1";
    	private $product_id = "8";
    	private $product_base = "loco-automatic-translate-addon-pro";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_loco-automatic-translate-addon-pro_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=loco-automatic-translate-addon-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
